...
//TODO lightining use multiple lights
void main(){
    //the ambient component
    vec3 ambient = ambientLight.colorAndIntensity.rgb * ambientLight.colorAndIntensity.a;
    //diffuse component
    vec3 norm = normalize(inNormal);
    vec3 lightDir = normalize(-directionalLight.direction[0]);
    float diff = max(dot(norm, lightDir), 0.0);
    vec3 diffuse = diff * directionalLight.colorAndIntensity[0].rgb * directionalLight.colorAndIntensity[0].a;
    //specular component TODO lightning: do specular
    vec3 viewDir = normalize(inCP - inFragPos);
    vec3 reflectDir = reflect(-lightDir, norm);
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), 32.0);
    vec3 specular = spec * directionalLight.colorAndIntensity[0].rgb * directionalLight.colorAndIntensity[0].a; 
    //combine results
