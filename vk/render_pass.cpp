...
namespace vk {
    ...
    RenderPass* RenderPass::FakeShadowMapPass(const std::string& name)
    {
        ...
        VkRenderPass fakeShadowMapRenderPass;
        if (vkCreateRenderPass(vk::Device::gDevice->GetDevice(), &renderPassInfo, nullptr, &fakeShadowMapRenderPass) != VK_SUCCESS) {
